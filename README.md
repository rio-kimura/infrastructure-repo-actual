# 🎓 卒業制作：インフラ自動構築・監視通知＆要塞化マルチコンテナNOCシステム 技術仕様書 (`infrastructure-repo`)
...
## 1. **フロントエンド：`noc_web` (nginx:1.25-alpine)**
   * 外部からの管理者アクセスを唯一待ち受けるリバースプロキシ。コンテナ内にビルトインされている初期画面を完全にバイパスし、ホスト側から独自のプロキシ転送設定（`/etc/nginx/conf.d/default.conf`）を読み込み専用（`:ro`）マウントすることで、不正な改ざんやインジェクション攻撃を遮断している。
## 2. **バックエンド：`noc_analyzer` (内製 Python / Flask)**
   * 厳格なセッション認証（ゲートキーパー設計）が実装されたコア・アプリケーション。未認証状態での裏口URL（`/report`, `/download_csv`）への直接アクセスを全てログイン画面へと強制送還するロジックを持つ。同じホスト上のPrometheus APIと連携し、監査メトリクスの解析、HTML動的レンダリング、およびCSVデータのオンデマンドエクスポートを司る。
...
* **OS: AlmaLinux release 9.x (linux/amd64)**
  * **選定理由**: エンタープライズLinuxの事実上の世界標準である「RHEL（Red Hat Enterprise Linux）9」の完全互換ディストリビューション。実務における長期サポート（LTS）、安定性、および高度なセキュリティ（SELinux等）を教授するために選定。
...
* **コンテナエンジン: Docker Engine / CLI 29.4.3 (build 56be731 / 055a478)**
* **コンテナランタイム: containerd.io 1.7.27**
  * **選定理由**: 開発班がPCローカル環境で検証に使用しているバージョンおよびビルド番号とサーバー環境を完全に同一化（パリティ担保）。「開発環境では動いたが、本番サーバーに載せたらライブラリやランタイムのマイナーバージョンの差で予期せぬエラーを起こす」という、デプロイメントの先祖返り事故を論理的に根絶する。
...
* **警備スクリプト: NOC統合通知ボットコンテナ (noc-integrated-bot:latest)**
  * **選定理由**: インフラ班内製のPythonコンテナ。3台のサーバーの負荷状況を常時パトロールし、リソースの突発的な枯渇（CPU/メモリ閾値超過）やサーバーの物理的ダウンを検知した瞬間、Discordの特定チャンネルへリッチなアラートカードを即座に自動POSTする。
...
万が一、アプリケーションや内製通知ボットのコードに未知のバグ（無限ループやメモリリーク等）が発生した場合でも、Server C全体のOSや他の最重要監視コンテナを巻き添えにしてサーバーがハングアップ（共倒れ）するのを防ぐため、Docker Composeにてリソースの上限を厳格に制限。
...
リポジトリ内のアセット配置を機能ごとに一元管理し、Ansibleの `copy` モジュールによって構造を維持したまま丸ごとServer Cへ一発同期させる設計を採用。これにより、ファイル配置ミスによるパスのすれ違いやコンテナの起動エラーを物理的に排除している。
...
└── roles/                     # 手順書を機能ごとにコンポーネント化（役割分担）して格納するAnsibleの作法フォルダ
    ...
    │   │       ├── Dockerfile # Flaskアプリを安全に軽量ビルドするマルチステージビルド設計図
    ...
    │   ├── tasks/
    │   │   └── main.yml       # 監視コンテナおよびNOCアセット一式の「構造維持デプロイ・一括起動」タスク
    │   └── templates/
    ...
    │       ├── noc_docker-compose.yml.j2 # NOCシステム(Nginx, AP, DB)のコンテナ間ネットワーク分離・マウント定義書
    ...
    ├── node_exporter/         # 各サーバーのハードウェア・OSリソースを外部に安全に送り出すメーターの管理用枠
    │   └── tasks/
    │       └── main.yml       # Node Exporterタスクの冪等性を管理するタスクベース
